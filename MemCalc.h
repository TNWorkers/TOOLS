...
inline double calc_memoryT (const Eigen::Tensor<Scalar,Rank> &T, MEMUNIT memunit_input=GB)
{
	double out=Scalar(1.);
	for (size_t leg=0; leg<Rank; leg++)
	{
		out *= T.dimension(leg);
	}
...
// N scalars
template<typename Scalar>
inline double calc_memory (size_t N, MEMUNIT memunit_input=GB)
{
